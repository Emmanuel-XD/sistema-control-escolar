...
function insert_alumno()
{
    ...
    $matriculaCheckQuery = "SELECT matricula FROM alumnos WHERE matricula = '$matricula'";
    $matriculaCheckResult = mysqli_query($conexion, $matriculaCheckQuery);

    if (mysqli_num_rows($matriculaCheckResult) > 0) {
        $response = array(
            'status' => 'matricula',
